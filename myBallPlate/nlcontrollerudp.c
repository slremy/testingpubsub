...
int uprocess(char server_ip[15], int server_port,char* url)
{
	...
	if (sendto(sock_fd, url, strlen(url), 0, (struct sockaddr *)&sock_addr, slen)==-1)		// Send URL to Model
		diep("sendto()");
	...
	n = recv(sock_fd,recvline,BUFLEN,0);								//Receive from Model
	recvline[n]=0;
...
void controlState(){
    ...
    snprintf(url, sizeof url,"/u?&value0=%.4f&value1=%.4f&t=%.6f&]",u_x,u_y,uclock()-t0);
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 3)
    {
        ...
        snprintf(filename,sizeof filename,"cudp_ball%s_%s_%s.txt",clientname, network, suffix);
        printf("\nLogging data to %s\n", filename);
