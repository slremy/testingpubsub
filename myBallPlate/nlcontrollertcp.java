...
public class nlcontrollertcp extends TimerTask{
    ...
    public int uprocess(String server_ip, int port, String control_action)
    {
        ...
        int datalen=0;
        try {
            out.write(control_action.getBytes());
            // Receive the same string back from the server
            if ( in.read(data, 0, data.length) == -1)
                throw new SocketException("Connection close prematurely");

            String inputLine = new String(data);
            response = inputLine.split("\\[")[1].split("\\]")[0].split(" ");

            if (response.length != mydata.length) {
                datalen = 0;
            }
            else {
                datalen = response.length;
                for (int i = 0; i < datalen; i++) {
                    mydata[i]=Double.parseDouble(response[i]);
                }
            }
        } catch (Exception e) {
    ...
    public nlcontrollertcp(String args[]){
        ...
        String myfilename;
        myfilename=String.format("jtcp%s_%s_%s.txt",clientname, network, suffix);
        System.out.format( "\nLogging data to %s\n", myfilename);
        try {
    ...
    public static void main(String args[]) throws Exception{
        ...
        if (args.length > 4)
        {
            nlcontrollertcp controller = new nlcontrollertcp(args);
